...
def test_delegate_task_background_accepts_json_string_batch_after_recovery(monkeypatch):
    """Stringified task arrays can dispatch as one background batch unit."""
    import json
    from unittest.mock import MagicMock, patch
    import tools.delegate_tool as dt
    ...
    assert parsed["goals"] == ["a", "b"]


def test_delegate_task_background_runs_inline_when_batch_dispatch_rejected(monkeypatch):
    """A capacity/schedule rejection falls back to sync execution without leaking the child."""
    import json
    from unittest.mock import MagicMock, patch
    import tools.delegate_tool as dt
    ...
    with patch.object(dt, "_build_child_agent", return_value=fake_child), \
         patch.object(dt, "_resolve_delegation_credentials", return_value=creds), \
         patch.object(dt, "_run_single_child", side_effect=fake_run_single_child), \
         patch("tools.async_delegation.dispatch_async_delegation_batch", return_value={"status": "rejected", "error": "capacity reached"}):
        out = dt.delegate_task(goal="bg task", background=True, parent_agent=parent)

    parsed = json.loads(out)
    assert parsed["results"][0]["status"] == "completed"
    assert parsed["results"][0]["summary"] == "bg task"
