...
- GET  /v1/runs/dispatch-key    — retrieve accepted run status by Idempotency-Key
...
class RunStatusStore:
    ...
    def __init__(self, db_path: str = None, max_size: int = 1000):
        ...
        with self._lock:
            ...
            # The dispatch record is intentionally co-stored with the pollable
            # run status. A successful insert means the run id is accepted and
            # recoverable after a process restart, before the agent task is
            # allowed to execute. Scope keeps multiplexed profiles from
            # accidentally reconciling another profile's key.
            self._conn.execute(
                """CREATE TABLE IF NOT EXISTS run_dispatches (
                    ...
                    PRIMARY KEY (scope, dispatch_key)
                )"""
            )
            self._conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_run_dispatches_run_id ON run_dispatches (run_id)"
            )
            self._conn.commit()
    ...
    def get(self, run_id: str) -> Optional[Dict[str, Any]]:
        """Read one persisted status for reconciliation after a restart."""
        with self._lock:
            row = self._conn.execute(
    ...
    def lookup_dispatch(self, scope: str, dispatch_key: str) -> Optional[Dict[str, str]]:
        """Return the immutable acceptance record for a scoped dispatch key."""
        with self._lock:
            row = self._conn.execute(
                "SELECT payload_hash, run_id FROM run_dispatches WHERE scope = ? AND dispatch_key = ?",
                (scope, dispatch_key),
            ).fetchone()
        if row is None:
            return None
        return {"payload_hash": str(row[0]), "run_id": str(row[1])}
    ...
    def claim_dispatch(
        ...
        *,
        ...
        """Atomically accept a keyed run or return the existing key owner.

        The status and immutable key mapping commit in one SQLite transaction.
        Callers may only schedule the background agent task after ``accepted``;
        a concurrent or post-proxy retry receives ``replay`` or ``conflict``
        together with the original run id instead of creating another run.
        """
        payload = json.dumps(initial_status, default=str)
        updated_at = float(initial_status.get("updated_at", time.time()) or time.time())
        now = time.time()
        with self._lock:
            try:
                ...
                if row is not None:
                    self._conn.rollback()
                    return ("replay" if hmac.compare_digest(str(row[0]), payload_hash) else "conflict", str(row[1]))
                self._conn.execute(
                    "INSERT INTO run_statuses (run_id, data, updated_at) VALUES (?, ?, ?)",
                    (run_id, payload, updated_at),
                )
                self._conn.execute(
                    "INSERT INTO run_dispatches (scope, dispatch_key, payload_hash, run_id, created_at) VALUES (?, ?, ?, ?, ?)",
                    (scope, dispatch_key, payload_hash, run_id, now),
                )
                self._conn.commit()
    ...
    def put(self, run_id: str, data: Dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            # Dispatch records have exactly the same retention window as their
            # pollable runs. Keeping a key after its run status was evicted
            # would replay an irretrievable id forever and make reconciliation
            # lie after bounded-store cleanup.
            self._conn.execute(
                "DELETE FROM run_dispatches WHERE run_id NOT IN (SELECT run_id FROM run_statuses)"
            )
            self._conn.commit()
    ...
    def delete(self, run_id: str) -> None:
        with self._lock:
            ...
            self._conn.execute("DELETE FROM run_dispatches WHERE run_id = ?", (run_id,))
            self._conn.commit()
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        # (PiB ``reasoning_override`` from the /v1/responses body) still
        # wins over request model_options and config.
        reasoning_config = (
            reasoning_override
    ...
    def _parse_run_reasoning_effort(self, body: Dict[str, Any]) -> tuple[Optional[Dict[str, Any]], Optional["web.Response"]]:
        ...
        if parsed is None:
            return None, web.json_response(
                _openai_error("Invalid reasoning_effort; expected none|minimal|low|medium|high|xhigh"),
                status=400,
            )
        return parsed, None

    def _parse_run_model_override(self, body: Dict[str, Any]) -> tuple[Optional[str], Optional["web.Response"]]:
        """Parse bare per-run model overrides (allowlisted models only).

        Requests that also send an explicit ``provider`` are Hermes-aware
        picker selections and are handled by ``_request_agent_overrides``
        instead of this allowlist path. Model route aliases are likewise
        left alone so ``_resolve_route`` can own them.
        """
        raw = body.get("model")
        if raw is None or raw == "":
            return None, None
        ...
        if model == self._model_name:
            return None, None
        # Explicit provider → requested_model/provider path, not allowlist override.
        if _clean_request_string(body.get("provider")):
            return None, None
        if self._resolve_route(model) is not None:
            return None, None
        if model not in self._run_model_allowlist():
            return None, web.json_response(
                _openai_error("Requested model is not allowlisted for per-run override"),
                status=400,
            )
        return model, None
    ...
    _RUN_DISPATCH_KEY_HEADER = "Idempotency-Key"
    _RUN_DISPATCH_KEY_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9._:-]{15,127}\Z")

    def _parse_run_dispatch_key(
        self, request: "web.Request", *, required: bool = False
    ) -> tuple[Optional[str], Optional["web.Response"]]:
        """Extract a bounded, log-safe idempotency key for /v1/runs.

        Keys are opaque caller-generated identifiers. Rejecting whitespace and
        controls rather than trimming them prevents a request that was never
        accepted from being silently reconciled under a different key.
        """
        raw = request.headers.get(self._RUN_DISPATCH_KEY_HEADER)
        if raw is None or raw == "":
            if required:
                return None, web.json_response(
                    _openai_error(
                        f"Missing {self._RUN_DISPATCH_KEY_HEADER} header",
                        code="missing_dispatch_key",
                    ),
                    status=400,
                )
            return None, None
        if not self._RUN_DISPATCH_KEY_RE.fullmatch(raw):
            return None, web.json_response(
                _openai_error(
                    f"Invalid {self._RUN_DISPATCH_KEY_HEADER} header",
                    code="invalid_dispatch_key",
                ),
                status=400,
            )
        return raw, None
    ...
    @staticmethod
    def _run_dispatch_payload_hash(body: Any) -> Optional[str]:
        """Return a canonical full-body fingerprint for keyed-run conflicts."""
        try:
    ...
    @staticmethod
    def _run_dispatch_scope() -> str:
        """Scope dispatch keys to the multiplexed profile serving this request."""
        return _api_request_profile.get() or "default"

    def _dispatch_replay_response(self, run_id: str, *, replayed: bool) -> "web.Response":
        status = self._run_statuses.get(run_id) or self._run_status_store.get(run_id)
        response = {
            "run_id": run_id,
            "status": str((status or {}).get("status") or "started"),
            "replayed": replayed,
        }
        return web.json_response(
            response,
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        dispatch_key, dispatch_key_err = self._parse_run_dispatch_key(request)
        if dispatch_key_err is not None:
            return dispatch_key_err
        ...
        dispatch_scope = self._run_dispatch_scope()
        payload_hash = None
        if dispatch_key:
            payload_hash = self._run_dispatch_payload_hash(body)
            if payload_hash is None:
                return web.json_response(
                    _openai_error("Invalid JSON payload for idempotent dispatch", code="invalid_dispatch_payload"),
                    status=400,
                )
            # Reconcile before the concurrency gate or request validation. A
            # caller that lost a proxy response must get the already accepted
            # id even if the gateway is now busy or mutable request context
            # (such as a working directory) has changed.
            existing = self._run_status_store.lookup_dispatch(dispatch_scope, dispatch_key)
            if existing is not None:
                if not hmac.compare_digest(existing["payload_hash"], payload_hash):
                    return web.json_response(
                        _openai_error(
                            "Idempotency-Key was already accepted with a different request payload",
                            code="idempotency_key_conflict",
                        ),
                        status=409,
                    )
                return self._dispatch_replay_response(existing["run_id"], replayed=True)
        ...
        # Replayed keys returned above intentionally bypass the cap because no
        # new agent work will be created. First-use keys are checked after
        # request validation below, before their acceptance is committed.
        ...
        if dispatch_key:
            try:
                claim, accepted_run_id = self._run_status_store.claim_dispatch(
                ...
                logger.exception("[api_server] failed to atomically persist dispatch acceptance")
                return web.json_response(
                    _openai_error("Unable to accept run dispatch; retry with the same Idempotency-Key", code="dispatch_store_unavailable"),
                    status=503,
                    headers={"Retry-After": "1"},
                )
            if claim == "conflict":
            ...
            if claim == "replay":
                return self._dispatch_replay_response(accepted_run_id, replayed=True)
        ...
        if dispatch_key:
            return self._dispatch_replay_response(run_id, replayed=False)
        return web.json_response(
    ...
    async def _handle_get_run_by_dispatch_key(self, request: "web.Request") -> "web.Response":
        """GET /v1/runs/dispatch-key — authenticated reconciliation by key."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        dispatch_key, key_err = self._parse_run_dispatch_key(request, required=True)
        if key_err is not None:
            return key_err
        record = self._run_status_store.lookup_dispatch(self._run_dispatch_scope(), dispatch_key or "")
        if record is None:
            return web.json_response(
                _openai_error("No accepted run for Idempotency-Key", code="dispatch_key_not_found"),
                status=404,
            )
        status = self._run_statuses.get(record["run_id"]) or self._run_status_store.get(record["run_id"])
        if status is None:
            # This should be unreachable because claim_dispatch stores both in
            # one transaction, but fail explicitly rather than inventing a
            # successful reconciliation when retention/storage was damaged.
            return web.json_response(
                _openai_error("Accepted run status is unavailable", code="dispatch_run_status_unavailable"),
                status=503,
            )
    ...
    async def inject_process_notification(self, synth_text: str, evt: Dict[str, Any], source: Any) -> None:
        """Persist an async delegation result through a follow-up session turn."""
        session_id = str(
